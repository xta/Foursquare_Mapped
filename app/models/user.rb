class User < ActiveRecord::Base
  ...
  has_many :checkins
	...
	def self.from_omniauth(auth)
	    if User.where(provider: auth.slice(:provider).provider, uid: auth.slice(:uid).uid).any?
			User.where(provider: auth.slice(:provider).provider, uid: auth.slice(:uid).uid).first
	  	else
		  	new_user = new
